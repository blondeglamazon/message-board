  ...
  async function handleSocialLogin(provider: 'google' | 'apple') {
    ...
    try {
      if (Capacitor.isNativePlatform()) {
        ...
        // 1. Sign in and get the session data
        ...
        // 2. 🚨 THE FIX: Force the session to be recognized by the app
        if (data?.session) {
          await supabase.auth.setSession(data.session)
        }

        // 3. 🚨 THE REDIRECT: Use window.location to force the app to reload onto the Home page
        // This stops the "Processing" spinner and ensures the UI updates.
